...
const { ProductModel, CategoryModel, TagModel, ProductTagModel } = require("../models");
...
router.get('/', async (req, res)=>{
    try{
        const products = await ProductModel.findAll({ include: [{ model: CategoryModel }, { model: TagModel }]});
...
router.get('/:id', async (req, res)=>{
    try{
        const productID = await ProductModel.findByPk(req.params.id, { include: [{ model: CategoryModel}, { model: TagModel }]})
        !productID ? res.status(404).json({ message: "ERROR Product ID not found!" }) : res.status(200).json(productID);
    } catch (err){
